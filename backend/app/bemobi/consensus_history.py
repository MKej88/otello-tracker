...
def build_consensus_history(
    ...
    *,
    ...
    for item in beat_miss:
        ...
        if metadata is None:
            continue
        model = _target_revision(dict(metadata["model_revision"]))
        result_date = str(metadata["result_date"])
        if model.get("after_date"):
            model["days_after_result"] = (
                date.fromisoformat(str(model["after_date"])) - date.fromisoformat(result_date)
            ).days
        events.append(
            {
                "period": period,
