...
def _preferred_price(connection, symbol: str, as_of_date: str):
    floor_date = (date.fromisoformat(as_of_date) - timedelta(days=MAX_MARKET_LOOKBACK_DAYS)).isoformat()
...
def _preferred_fx(connection, base: str, day: str):
    floor_date = (date.fromisoformat(day) - timedelta(days=MAX_MARKET_LOOKBACK_DAYS)).isoformat()
    return connection.execute(
        """
...
def _bemobi_market_attribution(
    connection,
    *,
    start_date: str,
    current_date: str,
