...
from app.economic_nav import _cash_fx_revaluation, _latest_cost_anchors, _nearest_fx, _option_values
...
def _component(key: str, label: str, amount_nok: Decimal, shares: int, formula: str, details: dict[str, Any] | None = None) -> dict[str, Any]:
    return {"key": key, "label": label, "amount_mnok": _float(amount_nok / MILLION), "per_share_nok": _float(amount_nok / Decimal(shares)), "formula": formula, "details": details or {}}
...
def _estimated_point(connection, day: str, database_path: str | None) -> dict[str, Any]:
    ...
    anchor = connection.execute("SELECT as_of_date FROM cash_anchors WHERE anchor_type='REPORTED' AND as_of_date<=? ORDER BY as_of_date DESC,id DESC LIMIT 1", (day,)).fetchone()
    if anchor is None:
    ...
    operating_cost_nok = Decimal(str(base_cost["amount_usd_decimal"])) / Decimal(int(base_cost["period_days_int"])) * Decimal(days_since_anchor) * Decimal(str(usd_nok["rate"]))
    ...
    return {"ready": True, "date": day, "nav_total_mnok": _float(estimated_total_nok / MILLION), "nav_per_share": _float(estimated_per_share), "otec_price": _float(otec_price), "discount_pct": _float(discount), "shares_outstanding": shares, "accounting_nav_per_share": _float(row.get("nav_per_share_nok")), "composition": composition, "reconciliation_residual_mnok": _float((estimated_total_nok - composition_total) / MILLION), "model": "ESTIMATED_NAV_V1"}
...
def estimated_nav_history(database_path: str | None = None, *, days: int) -> dict[str, Any]:
    ...
    with get_connection(database_path) as connection:
        latest = connection.execute("SELECT MAX(substr(as_of_at,1,10)) AS max_date FROM nav_snapshots WHERE calculation_version=? AND nav_scope='FULL'", (FULL_CALCULATION_VERSION,)).fetchone()
        ...
        rows = connection.execute("SELECT DISTINCT substr(as_of_at,1,10) AS date FROM nav_snapshots WHERE calculation_version=? AND nav_scope='FULL' AND substr(as_of_at,1,10)>=? AND substr(as_of_at,1,10)<=? ORDER BY date", (FULL_CALCULATION_VERSION, requested_start, current_date)).fetchall()
        predecessor = connection.execute("SELECT MAX(substr(as_of_at,1,10)) AS date FROM nav_snapshots WHERE calculation_version=? AND nav_scope='FULL' AND substr(as_of_at,1,10)<=?", (FULL_CALCULATION_VERSION, requested_start)).fetchone()
