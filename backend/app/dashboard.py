...
def dashboard_summary(database_path: str | None = None) -> dict[str, Any]:
    """Return the latest real dashboard KPIs from daily CORE NAV snapshots.

    No demo values are returned. A newly initialized database therefore reports
    ``not_ready`` until market-data backfill and the daily NAV rebuild have run.
    """
    with get_connection(database_path) as connection:
        rows = connection.execute(
            ...
            SELECT as_of_at, nav_per_share_nok, otec_price_nok, discount_pct,
                   ...
                   status, components_json, quality_notes
            FROM nav_snapshots
        ...
        if not rows:
            return {
                ...
                "message": "Kjør markedsdata-backfill og daglig NAV-rebuild for å fylle dashboardet.",
            }
        ...
        nav_change = _pct_change(
            ...
            previous["nav_per_share_nok"] if previous is not None else None,
        )
        otec_change = _pct_change(
            latest["otec_price_nok"],
            previous["otec_price_nok"] if previous is not None else None,
        )
        discount_change = None
        if previous is not None:
            discount_change = float(
                Decimal(str(latest["discount_pct"])) - Decimal(str(previous["discount_pct"]))
            )
        ...
        cash_change = _pct_change(
            ...
            previous["cash_estimate_nok"] if previous is not None else None,
        )
...
def dashboard_history(
    ...
    *,
    ...
    """Return a bounded daily CORE NAV/OTEC/discount series for dashboard charts."""
    ...
    with get_connection(database_path) as connection:
        ...
        rows = connection.execute(
            ...
            ORDER BY as_of_at
            """,
            (CALCULATION_VERSION, start_date.isoformat(), end_date.isoformat()),
        ).fetchall()
