-- Backfill the first OTEC activity gap after the curated official Euronext export ended.
-- The volume rows below were manually verified against the public FT Markets historical
-- table and are intentionally stored under a separate non-official source. Runtime rows
-- after this migration are written from Euronext's official delayed trade files.
...
-- Backfill the current programme's disclosed maximum purchase price from the official
-- Euronext programme announcement instead of waiting for a later NewsWeb revalidation.
INSERT INTO source_documents(
    source_id, external_id, document_type, title, published_at, url, metadata_json
)
SELECT s.id,
