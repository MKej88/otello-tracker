...
from app.estimated_nav_history import _share_count_driver
...
def _nav_effect(nav_snapshot, latest, shares) -> dict[str, float | None]:
    ...
    bought = Decimal(str(latest["cumulative_program_shares"] or 0))
    spent = Decimal(str(latest["cumulative_program_amount_nok"] or 0))
    outstanding = Decimal(str(shares["outstanding_shares"] or 0))
    nav_total = Decimal(str(nav_snapshot["nav_total_nok"] or 0))
...
def buyback_dashboard(
    ...
    *,
    ...
    with get_connection(database_path) as connection:
        ...
        nav_snapshot = connection.execute(
            """
        ...
        if latest is not None and latest["start_date"]:
            start_nav_snapshot = connection.execute(
                """
                SELECT nav_total_nok
                FROM nav_snapshots
...
def buyback_overview_status(
    database_path: str | None = None, *, as_of_date: str | None = None
) -> dict[str, Any]:
    """Return only the buyback fields rendered on the overview page."""
    dashboard = buyback_dashboard(database_path, as_of_date=as_of_date)
    return {
