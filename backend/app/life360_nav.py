...
def _lif_price(connection, as_of_date: str) -> dict[str, Any] | None:
    return _market_price(connection, as_of_date, symbol="LIF", currency="USD")
...
def _usd_nok(connection, as_of_date: str) -> dict[str, Any] | None:
    return _fx_rate(connection, as_of_date, base_currency="USD")
...
def _life360_holding(connection, as_of_date: str) -> dict[str, Any] | None:
    row = connection.execute(
        """
...
def life360_market_value(
    *,
    as_of_date: str,
    database_path: str | None = None,
) -> dict[str, Any]:
    """Source-backed historical Life360 market value for attribution only.

    Before the Nasdaq listing, Life360 traded on ASX as 360 CDIs. Three CDIs
    represent one common share, so the historical market value uses three 360.AX
    units per Otello common share and AUD/NOK. From 6 June 2024 the calculation
    uses LIF common shares and USD/NOK directly.

    This helper does not restate accounting NAV. Pre-2025 Otello reports carried
    Life360 and Alliance Venture Spring together at cost, so this market value is
    an analytical attribution layer only.
    """
    ...
    with get_connection(database_path) as connection:
        holding = _life360_holding(connection, as_of_date)
        price = _market_price(
            connection,
            as_of_date,
            symbol=symbol,
        ...
        fx = _fx_rate(connection, as_of_date, base_currency=currency)
...
def life360_nav_adjustment(
    *,
    ...
    with get_connection(database_path) as connection:
        ...
        current_holding = _life360_holding(connection, as_of_date)
        anchor_holding = _life360_holding(connection, anchor_date)
