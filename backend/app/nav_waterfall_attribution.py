...
from app.life360_nav import _usd_nok, life360_nav_adjustment
from app.nav.daily_nav import _holding, _nearest_fx, _preferred_price
from app.nav_waterfall_settlement import nav_waterfall_summary as base_nav_waterfall_summary
...
def _bemobi_attribution(
    ...
    *,
    ...
    anchor_price = _preferred_price(connection, "BMOB3", anchor_date)
    current_price = _preferred_price(connection, "BMOB3", as_of_date)
    anchor_fx = _nearest_fx(connection, "BRL", anchor_date)
    current_fx = _nearest_fx(connection, "BRL", as_of_date)
    anchor_holding = _holding(connection, anchor_date)
...
def _life360_attribution(*, as_of_date: str, database_path: str | None) -> dict[str, Any]:
    ...
    with get_connection(database_path) as connection:
        anchor_fx_row = _usd_nok(connection, anchor_date)
