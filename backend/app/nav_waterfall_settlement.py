...
from app.nav_waterfall_investor import nav_waterfall_summary as investor_waterfall_summary
from app.option_settlement import MILLION, nav_cash_settlement, settlement_inputs_from_daily_row
...
def _option_row(connection, day: str):
...
def nav_waterfall_summary(database_path: str | None = None) -> dict[str, Any]:
    result = investor_waterfall_summary(database_path)
    if not result.get("ready"):
        return result
    ...
    with get_connection(database_path) as connection:
        anchor_inputs = settlement_inputs_from_daily_row(_option_row(connection, anchor_date))
        current_inputs = settlement_inputs_from_daily_row(_option_row(connection, as_of_date))
