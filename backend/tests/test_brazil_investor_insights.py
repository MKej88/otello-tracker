...
def _focus_payload(survey_date: str, *, selic: float, ipca: float, gdp: float, usd_brl: float) -> dict:
    return {
        "value": [
            {"Indicador": "Selic", "Data": survey_date, "DataReferencia": "2027", "Mediana": selic},
            {"Indicador": "IPCA", "Data": survey_date, "DataReferencia": "2027", "Mediana": ipca},
            {"Indicador": "PIB Total", "Data": survey_date, "DataReferencia": "2027", "Mediana": gdp},
            {"Indicador": "Câmbio", "Data": survey_date, "DataReferencia": "2027", "Mediana": usd_brl},
        ]
    }


def test_focus_trend_compares_same_official_series_7d_and_30d_back() -> None:
    async def fetcher(url: str, **_kwargs: object) -> _Response:
        ...
        if "Data le '2026-08-29'" in filter_text:
            return _Response(_focus_payload("2026-08-28", selic=11.75, ipca=4.10, gdp=1.50, usd_brl=5.25))
        if "Data le '2026-08-06'" in filter_text:
            return _Response(_focus_payload("2026-08-06", selic=12.00, ipca=4.25, gdp=1.40, usd_brl=5.30))
        raise AssertionError(filter_text)

    current_focus = {
        "values": {
            "selic": {"2027": {"median": 11.50, "survey_date": "2026-09-05"}},
            "ipca": {"2027": {"median": 4.00, "survey_date": "2026-09-05"}},
            "gdp": {"2027": {"median": 1.60, "survey_date": "2026-09-05"}},
            "usd_brl": {"2027": {"median": 5.20, "survey_date": "2026-09-05"}},
        }
    }
