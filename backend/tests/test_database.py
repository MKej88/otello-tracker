...
def test_migrations_are_idempotent_and_seed_reference_data(tmp_path) -> None:
    ...
    assert status["latest_migration"] == "0018"
    ...
    assert status["table_counts"]["shareholder_snapshots"] == 0
    assert status["table_counts"]["shareholder_snapshot_rows"] == 0
    ...
    with get_connection(database_path) as connection:
        ...
        assert connection.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='shareholder_snapshots'").fetchone()[0] == 1
        assert connection.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='shareholder_snapshot_rows'").fetchone()[0] == 1
...
def test_database_status_api_initializes_schema(tmp_path) -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            assert payload["latest_migration"] == "0018"
            ...
            assert payload["table_counts"]["shareholder_snapshots"] == 0
            assert payload["table_counts"]["shareholder_snapshot_rows"] == 0
