...
def test_migrations_are_idempotent_and_seed_reference_data(tmp_path) -> None:
    ...
    assert status["latest_migration"] == "0037"
    assert status["table_counts"]["sources"] == 19
    assert status["table_counts"]["instruments"] == 4
    assert status["table_counts"]["bemobi_investor_facts"] == 19
    ...
    with get_connection(database_path) as connection:
        ...
        life360_holding_columns = {
            row["name"] for row in connection.execute("PRAGMA table_info(life360_holding_anchors)")
        }
        assert {
...
def test_database_status_api_initializes_schema(tmp_path) -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            assert payload["latest_migration"] == "0037"
            assert payload["table_counts"]["sources"] == 19
            assert payload["table_counts"]["instruments"] == 4
            assert payload["table_counts"]["bemobi_investor_facts"] == 19
