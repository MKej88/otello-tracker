...
def test_html_preloads_first_screen_data_for_direct_routes() -> None:
    ...
    assert '"#brasil": [' in source
    assert '["/api/dashboard/economic", "low"]' in source
...
def test_navigation_starts_buyback_data_in_parallel_with_route_code() -> None:
    ...
    buyback_block = app_source[buyback_preload:].split("}", maxsplit=1)[0]

    assert buyback_preload < buyback_route
    assert 'preloadJson("/api/buybacks/dashboard")' in buyback_block
    assert "/api/bemobi/dashboard" not in buyback_block
    assert (
        'fetchPreloadedJson<Dashboard>("/api/buybacks/dashboard")'
        in buyback_source
    )
    assert "/api/bemobi/dashboard" not in buyback_source
