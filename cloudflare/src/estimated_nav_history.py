...
async def _cash_breakdown(
    repository, *, start_date: str, current_date: str
) -> dict[str, Any]:
...
async def _estimated_point(repository, day: str) -> dict[str, Any]:
    ...
    cash_fx = await _cash_fx_revaluation(
        repository, cash_anchor_date=cash_anchor_date, as_of_date=day
    )
...
async def _change(
    repository, start: dict[str, Any], current: dict[str, Any], requested_start: str
) -> dict[str, Any]:
...
def _stored_point(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "accounting_nav_per_share": row.get("accounting_nav_per_share_nok"),
        "composition": json.loads(str(row["composition_json"])),
        "reconciliation_residual_mnok": row.get("reconciliation_residual_mnok"),
        "model": ESTIMATED_NAV_CALCULATION_VERSION,
    }


async def materialize_estimated_nav_history(
    repository, *, batch_size: int = 100
) -> dict[str, Any]:
    """Materialize one restart-safe batch of missing historical dates."""
    rows = await repository.all(
        """SELECT DISTINCT substr(n.as_of_at, 1, 10) AS date
           FROM nav_snapshots n LEFT JOIN estimated_nav_history_points p
...
async def estimated_nav_history(
    ...
    latest = await repository.first(
        "SELECT MAX(date) AS max_date FROM estimated_nav_history_points WHERE calculation_version=? AND quality='VALID'",
        (ESTIMATED_NAV_CALCULATION_VERSION,),
    )
    current_date = latest.get("max_date") if latest else None
    ...
    rows = await repository.all(
        "SELECT * FROM estimated_nav_history_points WHERE calculation_version=? AND quality='VALID' AND date>=? AND date<=? ORDER BY date",
        (ESTIMATED_NAV_CALCULATION_VERSION, requested_start, current_date),
    )
    predecessor = await repository.first(
        "SELECT * FROM estimated_nav_history_points WHERE calculation_version=? AND quality='VALID' AND date<=? ORDER BY date DESC LIMIT 1",
    ...
    return {
        ...
        "note": "Estimert NAV leses fra validert, materialisert historikk.",
    }
