...
async def discount_history(repository, *, days: int = 365, max_points: int = 600) -> dict[str, Any]:
    ...
    periods = await repository.all("SELECT id,period_start,trade_date AS period_end,shares AS weekly_shares,treasury_shares_after FROM buybacks WHERE period_start IS NOT NULL AND treasury_shares_after IS NOT NULL AND trade_date>=? AND period_start<=? ORDER BY period_start,trade_date,id", (history["from"], history["to"]))
    transactions = await repository.all("SELECT d.id,d.weekly_buyback_id,d.trade_date,d.shares,d.quality FROM buyback_daily_transactions d JOIN buybacks b ON b.id=d.weekly_buyback_id WHERE b.period_start IS NOT NULL AND b.trade_date>=? AND b.period_start<=? ORDER BY d.weekly_buyback_id,d.trade_date,d.id", (history["from"], history["to"]))
    share_counts = await repository.all("SELECT id,effective_from,total_shares,treasury_shares,outstanding_shares FROM otello_share_counts WHERE effective_from<=? ORDER BY effective_from,id", (history["to"],))
    daily_rows = _apply_buyback_share_adjustments([dict(row) for row in daily_rows], [dict(row) for row in periods], [dict(row) for row in transactions], [dict(row) for row in share_counts])
    ...
    return {
        ...
        "current_economic": await _economic_reference(repository), "points": points, "estimated": estimated,
    }