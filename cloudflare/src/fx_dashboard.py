...
MAX_LOOKBACK_DAYS = 7
SOURCE_PRIORITY = {"NORGES_BANK": 0, "ECB": 1}
SOURCE_URLS = {
...
def _period_snapshot(
    *,
    ...
    current_usd = _on_or_before(
        usd_series, current_date, max_lookback_days=MAX_LOOKBACK_DAYS
    )
    ...
    if current_usd is not None and reference_usd is not None:
        ...
        usd_brl_pct = _pct_change(
            current_usd_rate / current_brl_rate,
            reference_usd_rate / reference_brl_rate,
        )
...
async def fx_dashboard(repository) -> dict[str, Any]:
    ...
    if latest is None or latest.get("max_date") is None:
    ...
    brl_series = await _preferred_series(
        repository,
        base_currency="BRL",
        start_date=start_date,
        end_date=end_date,
    )
    usd_series = await _preferred_series(
        repository,
        base_currency="USD",
        start_date=start_date,
        end_date=end_date,
    ...
    current_usd = _on_or_before(
        usd_series, end_date, max_lookback_days=MAX_LOOKBACK_DAYS
    )
    ...
    return {
        ...
        "sources": [
            ...
            for code in sorted(
                source_codes, key=lambda code: SOURCE_PRIORITY.get(code, 5)
            )
        ],
        "method_note": (
