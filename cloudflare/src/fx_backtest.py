...
async def _outcomes(repository) -> list[dict[str, Any]]:
    rows = await repository.all(
        """
        SELECT id, metadata_json FROM source_documents
...
def _value_usd(balances: dict[str, Decimal], usd_nok: Decimal, brl_nok: Decimal) -> Decimal:
    return balances["USD"] + balances["NOK"] / usd_nok + balances["BRL"] * brl_nok / usd_nok
...
async def _movements(repository, start: str, end: str) -> list[dict[str, Any]]:
...
async def _period_backtest(repository, outcome: dict[str, Any], anchors: dict[str, dict[str, Any]]) -> dict[str, Any]:
...
async def fx_backtest_summary(repository) -> dict[str, Any]:
    anchors = await _anchors(repository)
    outcomes = await _outcomes(repository)
    if not outcomes:
        return {"ready": False, "reason": "missing_reported_fx_outcomes"}
    periods = [await _period_backtest(repository, outcome, anchors) for outcome in outcomes]
