...
from estimated_nav_history import estimated_nav_history as _estimated_nav_history
...
async def estimated_nav_history(repository, *, days: int) -> dict[str, Any]:
    """Present Life360 as a gross listed holding without changing calculated NAV.

    The underlying investor model continues to apply only the mark-to-market adjustment:
    current LIF value minus the fair value already embedded in reported ONA. This wrapper
    reclassifies that embedded value out of ONA for the user-facing composition and change
    bridge, so the Life360 row shows the actual holding value rather than only the delta.
    """
    ...
    current_state = (
        await life360_nav_adjustment(repository, as_of_date=current_date)
        if current_date
    ...
    if change.get("ready"):
        ...
        start_state = (
            await life360_nav_adjustment(repository, as_of_date=start_date)
            if start_date
            else {"ready": False, "reason": "missing_start_date"}
        )
        end_state = current_state if end_date == current_date else (
            await life360_nav_adjustment(repository, as_of_date=end_date)
            if end_date
            else {"ready": False, "reason": "missing_current_date"}
        )
