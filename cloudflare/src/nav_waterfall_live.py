...
from nav_refresh import calculate_core_nav
from nav_waterfall import (
...
def _synthesized_option_values(ona: dict[str, Any]) -> tuple[Decimal, Decimal] | None:
    accounting = Decimal(str(ona.get("option_liability_nok") or "0"))
    inputs = _json_object(ona.get("option_inputs_json"))
    gross_raw = inputs.get("gross_fair_value_nok")
    if gross_raw is None:
        fair_raw = ona.get("option_fair_value_per_option_nok")
...
async def _synthesize_report_anchor(repository, anchor_date: str):
    core = await calculate_core_nav(repository, anchor_date)
...
async def _resolve_anchor(repository, anchor_date: str):
    stored = await _full_row(repository, anchor_date)
    if stored is not None:
        option_values = _option_values(_components(stored.get("components_json")))
        if option_values is None:
            return None, None, {
                "ready": False,
                "reason": "missing_anchor_option_values",
    ...
    return await _synthesize_report_anchor(repository, anchor_date)
...
async def nav_waterfall_summary(repository) -> dict[str, Any]:
    ...
    anchor_row, anchor_options, anchor_resolution = await _resolve_anchor(repository, anchor_date)
