...
def settlement_inputs_from_daily_row(row: Any) -> tuple[Decimal, int, Decimal] | None:
    if row is None:
        return None
    inputs = _object(row.get("option_inputs_json") if isinstance(row, dict) else row["option_inputs_json"])
    count_raw = inputs.get("option_count")
    strike_raw = row.get("option_strike_nok") if isinstance(row, dict) else row["option_strike_nok"]
    liability_raw = row.get("option_liability_nok") if isinstance(row, dict) else row["option_liability_nok"]
    try:
