...
async def _period_interest_income(
    repository,
    *,
    start_date: str,
    current_date: str,
            ...
            segments.append(
                {
                    ...
                    "source_document_id": int(row.get("id") or 0),
...
async def estimated_nav_history(
    ...
    if change.get("ready") and start_date:
        ...
        period_interest = await _period_interest_income(
            repository,
            start_date=start_date,
            current_date=current_date,
        )
        if period_interest.get("ready"):
            ...
            change["period_interest_income_status"] = period_interest
