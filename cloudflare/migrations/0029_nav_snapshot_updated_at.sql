-- Preserve the original row creation time while tracking later in-place NAV recalculations.
-- Dirty-state CORE/FULL refreshes use UPSERT on the same business-day row, so created_at
-- alone cannot represent the latest calculation after the first insert.
