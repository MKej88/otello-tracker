...
--
-- Source documents are inserted only for an in-place populated production upgrade.
-- A fresh D1 receives the same canonical rows from the deterministic bootstrap, so
-- seeding them here as well would create source_document ID collisions during CI/bootstrap.
